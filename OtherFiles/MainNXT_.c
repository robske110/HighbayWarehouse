...
#pragma config(Sensor, S4,     					VERTICAL,    		 sensorTouch)
#pragma config(Sensor, S3,     					HORIZONTAL_T,    		 sensorTouch)
...
void moveHorizontal(const string UpDown)
{
	...
	else
	{
		...
		else
		{
			...
			{
				motor[HORIZONTAL] = Horizontal.Speed;
				while(SensorValue(HORIZONTAL_T) != 1){
				}
				motor[HORIZONTAL] = 0;
			}
			else
...
const short sensorVERTICALShaftOffset = 8;
typedef struct{
	short nullOffset;
	short shaftOffset;
}sensor;
	...
	if(ForwardBackward == "Forward")
	{
		...
		if(ForwardBackward == "Backward")
		{
			...
			while(!ExitLoop)
			{
				...
				{
					ExitLoop = true;
					error("EMD::AS","ExceededMaxDistance","moveVertical::B",true);
				}
			}
			motor[VERTICAL] = 0;
		}
